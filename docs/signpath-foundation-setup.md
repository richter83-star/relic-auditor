...
The manual signing workflow requires `Valid`, a trusted timestamp, and the
case-sensitive simple name `Dracanus AI`. If SignPath Foundation issues a
certificate with a different simple name, do not weaken or bypass the check.
Update the pinned updater publisher, documentation, and tests in a separately
reviewed change first; then rebuild from the new `main` head.
...
5. reject an invalid, untimestamped, unchanged, or wrong-publisher result;
