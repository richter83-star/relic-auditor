...
param(
    ...
    [string]$ExpectedSourceSha256 = "a6959b5747287785196f7319cb097f10621fa06df457727546c62edee8bb819a",
...
if (-not $SourceArchive) {
    $SourceArchive = Join-Path $KitRoot "releases\relic-auditor-0.8.1.zip"
}
...
if (-not [Environment]::Is64BitOperatingSystem) {
    throw "Relic Auditor 0.8.1 supports 64-bit Windows only."
}
...
$SourceRoot = Join-Path $UnpackedRoot "relic-auditor-0.8.1"
...
if ($ProjectMetadata -notmatch '(?m)^version = "0\.8\.1"$') {
    throw "The source archive does not identify itself as Relic Auditor 0.8.1."
}
...
if ($LASTEXITCODE -ne 0 -or $VersionOutput -ne "relic 0.8.1") {
...
if ($LASTEXITCODE -ne 0 -or $BundledVersion -ne "relic 0.8.1") {
...
$InstallerExe = Join-Path $OutputDirectory "Relic-Auditor-Setup-0.8.1-x64.exe"
...
try {
    ...
    if ($LASTEXITCODE -ne 0 -or $InstalledVersion -ne "relic 0.8.1") {
