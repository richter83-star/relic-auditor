...
param(
    ...
    [string]$ExpectedSourceSha256 = "45d06bd992c4acb8b7de4414443c354f1094cf77ec0d9ca80c7082789619c5a4",
...
if (-not $SourceArchive) {
    $SourceArchive = Join-Path $KitRoot "releases\relic-auditor-0.10.0.zip"
}
...
if (-not [Environment]::Is64BitOperatingSystem) {
    throw "Relic Auditor 0.10.0 supports 64-bit Windows only."
}
...
Get-ChildItem -LiteralPath $OutputDirectory -File -ErrorAction SilentlyContinue |
    Where-Object { $_.Name -in @(
        "Relic-Auditor-Setup-0.10.0-x64.exe",
        "SHA256SUMS.txt",
...
$SourceRoot = Join-Path $UnpackedRoot "relic-auditor-0.10.0"
...
if ($ProjectMetadata -notmatch '(?m)^version = "0\.10\.0"$') {
    throw "The source archive does not identify itself as Relic Auditor 0.10.0."
}
...
if ($LASTEXITCODE -ne 0 -or $VersionOutput -ne "relic 0.10.0") {
...
if ($LASTEXITCODE -ne 0 -or $BundledVersion -ne "relic 0.10.0") {
...
$InstallerExe = Join-Path $OutputDirectory "Relic-Auditor-Setup-0.10.0-x64.exe"
...
try {
    ...
    if ($LASTEXITCODE -ne 0 -or $InstalledVersion -ne "relic 0.10.0") {
