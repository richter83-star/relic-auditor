...
class ResurrectionTests(unittest.TestCase):
    ...
    def test_02_insufficient_nodes_emits_toss_it(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            audit = audit_estate(root)
            truth = analyze_technical_truth(audit)
            result = resurrect_estate(audit, truth, ResurrectionConfig(min_subgraph_nodes=3))
            self.assertEqual(result.verdict, "TOSS_IT")
            self.assertTrue(result.gate.bypass_llm)
    ...
    def test_03_connected_real_core_emits_resurrect(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertTrue(len(result.blueprint.salvageable_core_paths) > 0)
            self.assertTrue(result.verdict_confidence >= 0.60)
    ...
    def test_05_citation_verification_detects_hallucinations(self):
        envelope = {
            ...
            "substantive_symbols": [{"symbol_id": "sym_real", "name": "real_fn", "file": "src/real.py"}],
            "subgraph_id": "subgraph_123",
            "surface_anchors": [],
        }
        # LLM response inventing a file that doesn't exist in envelope
        fake_llm_response = {
            "verdict": "RESURRECT",
            "salvageable_core_paths": ["src/real.py", "src/fake_magic_ai.py"],
            "citations": [{"claim": "AI magic works", "source_evidence_ids": ["sym_real", "sym_hallucinated"]}],
        }
        ...
        self.assertTrue(any("fake_magic_ai.py" in c for c in verification.ungrounded_claims))
        self.assertTrue(any("sym_hallucinated" in c for c in verification.ungrounded_claims))

    def test_06_cli_resurrect_command(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
    ...
    def test_07_market_context_segregation(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            ...
            audit = audit_estate(root)
            truth = analyze_technical_truth(audit)
            result = resurrect_estate(audit, truth, ResurrectionConfig(include_market_facts=True))
            self.assertEqual(result.verdict, "RESURRECT")
            self.assertIsNotNone(result.market_context)
            self.assertEqual(result.market_context.epistemic_rating, "external_market_speculation")
            self.assertIn("Static Application Security Testing", result.market_context.target_category)
            self.assertTrue(len(result.market_context.active_competitors) > 0)
