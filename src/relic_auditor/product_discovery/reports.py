...
def write_product_reports(result: DiscoveryResult, output: Path) -> list[Path]:
    ...
    _json(
        ...
        {
            "schema_version": "1.0",
            "intent_reconstruction": result.intent,
            "project_families": result.project_families,
            "capabilities": result.capabilities,
        },
    )
    _json(
        paths["evidence"], {"schema_version": "1.0", "evidence": result.evidence_index}
    )
    _json(
        paths["extraction"], {"schema_version": "1.0", "plans": result.extraction_plans}
    )
...
def _brief(result: DiscoveryResult) -> str:
    ...
    revenue = next(
        (
            ...
            if "Highest probability of near-term revenue" in o["rank_labels"]
        ),
        top,
    )
    surprising = next(
        (
            o
            for o in opportunities
            if "Most surprising hidden product" in o["rank_labels"]
        ),
        top,
    )
    lines = [
        ...
        "",
        result.intent["actual_implementation_state"],
        "",
        "## Most valuable reusable capabilities",
        "",
    ]
    lines += [
        f"- **{c['name']}** — {c['description']} ({c['completion_level']}% completion signal)"
        for c in result.capabilities[:8]
    ]
    ...
    if opportunities:
        ...
        for index, o in enumerate(opportunities, 1):
            lines.append(
                f"| {index} | {o['title']} | {o['overall_score']} | {o['evidence_score']} | {o['extraction_effort']} | {o['reject_reason']} |"
            )
    else:
        lines.append("No proposal passed the evidence quality gate.")
    for heading, item in (
        ("Recommended primary product", top),
        ("Recommended near-term revenue product", revenue),
        ("Most surprising product opportunity", surprising),
    ):
        lines += ["", f"## {heading}", ""]
        lines.append(
            f"**{item['title']}** — {item['summary']}"
            if item
            else "No supported recommendation."
        )
    if top:
        lines += [
            "",
            "## Extraction plan",
            "",
            f"- Reuse: {', '.join(top['extraction_plan']['reuse'])}",
            f"- Rewrite: {', '.join(top['extraction_plan']['rewrite'])}",
            f"- Relative effort: **{top['extraction_effort']}**",
            "",
            "## GTM plan",
            "",
            top["gtm"]["positioning"],
            ...
            "",
            f"- Experiments: {', '.join(top['gtm']['validation_30_day']['experiments'])}",
            f"- Success: {top['gtm']['validation_30_day']['success']}",
            f"- Kill: {top['gtm']['validation_30_day']['kill']}",
            f"- Pivot: {top['gtm']['validation_30_day']['pivot']}",
            "",
            "## Risks and counterarguments",
            "",
        ] + [f"- {risk}" for risk in top["risks"]]
        lines += [
            "",
            ...
            "",
        ] + [f"- {u}" for u in top["unknowns"]]
    lines += ["", "## Repository evidence appendix", ""]
    lines += [
        f"- `{e['evidence_id']}` — `{e['path']}` lines {e['lines']} ({e['evidence_type']}, confidence {e['confidence']})"
        for e in result.evidence_index
    ]
    lines.append("")
    return "\n".join(lines)
...
def _gtm(result: DiscoveryResult) -> str:
    lines = [
        "# Relic GTM Proposals",
        "",
        "> Pricing and market claims are hypotheses until externally validated.",
        "",
    ]
    for o in result.opportunities:
        ...
        lines += [
            ...
            o["gtm"]["positioning"],
            "",
            f"**Offer:** {o['gtm']['offer_model']} — {o['gtm']['deliverable']}",
            "",
            ...
            "",
            f"**Primary channel:** {o['gtm']['channels']['primary']}",
            "",
            "### Landing page",
            "",
            sales["headline"],
        ...
        lines += [
            ...
            "### Discovery",
            "",
            sales["discovery_opening"],
            "",
        ] + [f"- {q}" for q in sales["discovery_questions"]]
        lines += [
            "",
            ...
            f"- Kill: {validation['kill']}",
            f"- Pivot: {validation['pivot']}",
            "",
        ]
    return "\n".join(lines)
